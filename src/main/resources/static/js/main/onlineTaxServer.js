	...
	$.ajax({
		...
		success : function(data) {
			if (typeof (isIndex) == 'undefined') {
				...
				// 点击年月日加载
				$("#pageIframe").contents().find("#online_total_count").html(forInt(data.YWSLZL));
	...
	$.ajax({
		...
		success : function(data) {
			if (typeof (isIndex) == 'undefined') {
				...
				// 点击年月日加载
				$("#pageIframe").contents().find("#online_current_num").html(forInt(data.VALUE));
...
function stationOnlineTaxServerCountLine(isIndex) {
	$.ajax({
		...
		success : function(data) {
			...
			var chart;
			if (typeof (isIndex) == 'undefined') {
				chart = echarts.init(document.getElementById('station_onlineTaxServerCount_line'));
			} else {
				var obj = document.getElementById("pageIframe").contentWindow.document.getElementById("station_onlineTaxServerCount_line");
				chart = echarts.init(obj);
			}
			chart.setOption({
				title : {
